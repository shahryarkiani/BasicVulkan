...
private:
	...
	void createGraphicsPipeline() {
		...
		vk::PipelineShaderStageCreateInfo fragShaderStageInfo(
			{},
			vk::ShaderStageFlags::BitsType::eFragment,
			fragShaderModule,
			"main"
		);
	...
	[[nodiscard]] vk::ShaderModule createShaderModule(const std::vector<char>& shaderCode) const {
	...
	void createSwapChain() {
		...
		if (queueIndices.graphicsFamily != queueIndices.presentFamily) {
			queueIndicesList = { queueIndices.graphicsFamily.value(), queueIndices.presentFamily.value() };
	...
	void createInstance() {
		if (enableValidationLayers && !checkValidationLayerSupport()) {
			throw std::runtime_error("Missing support for validation layers");
		}
	...
	bool isDeviceSuitable(vk::PhysicalDevice physicalDevice) {
		...
		return indices.isComplete() && swapChainSupport.isSuitable() && extensionsSupported;
	}
	...
	[[nodiscard]] QueueFamilyIndices findQueueFamilies(vk::PhysicalDevice device) const {
		...
		for (uint32_t i = 0; i < queueFamilies.size(); i++) {
			...
			bool supportsPresent = device.getSurfaceSupportKHR(i, surface) == vk::True;
			if (supportsPresent) indices.presentFamily = i;
		}
